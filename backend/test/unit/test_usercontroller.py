...
def test_get_user_by_email_multiple_users(mock_dao):
    """Multiple users: returns first user and calls dao.find correctly."""
...
def test_get_user_by_email_multiple_users_logs_and_returns_first(mock_dao, capsys):
    """Multiple users: logs warning AND still returns first user."""
    u1 = DummyUser("existing@example.com")
    u2 = DummyUser("existing@example.com")
    mock_dao.find.return_value = [u1, u2]
